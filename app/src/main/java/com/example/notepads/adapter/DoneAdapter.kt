...
import com.example.notepads.databinding.ListItemDoneBinding
...
class DoneAdapter(
    ...
    ) : RecyclerView.Adapter<DoneAdapter.DoneViewHolder>(), Filterable {

    private var allItemNotes: ArrayList<RecyclerNotesModel>

    init {
        allItemNotes = dao.getNotesForRecycler(DBHelper.FALSE_STATE, DBHelper.TRUE_STATE)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): DoneViewHolder {
        ...
        return DoneViewHolder(
            ListItemDoneBinding.inflate(LayoutInflater.from(context), parent, false)
        )
    }
    ...
    inner class DoneViewHolder(
        private val binding: ListItemDoneBinding
        ...
        fun saveData(data: RecyclerNotesModel) {
            ...
            binding.ImageViewCloseNote.setOnClickListener {
                dao.editState(data.id.toString(), DBHelper.FALSE_STATE, DBHelper.NOTES_CHECK_STATE)
                allItemNotes.removeAt(adapterPosition)
                notifyItemRemoved(adapterPosition)
